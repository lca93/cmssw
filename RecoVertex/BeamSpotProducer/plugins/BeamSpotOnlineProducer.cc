...
private:
  ...
  const BeamSpotOnlineObjects getBeamSpotFromRecord(const TokenType& token, const edm::Event& event, const edm::EventSetup& setup);
...
template <typename TokenType>
const BeamSpotOnlineObjects BeamSpotOnlineProducer::getBeamSpotFromRecord(const TokenType& token, const Event& event, const EventSetup& setup){
  auto bs = setup.getData(token);
  if (bs.sigmaZ()     < sigmaZThreshold_  ||
      bs.beamWidthX() < sigmaXYThreshold_ ||
      bs.beamWidthY() < sigmaXYThreshold_ ||
...
const BeamSpotOnlineObjects& BeamSpotOnlineProducer::chooseBS(const BeamSpotOnlineObjects& bs1, const BeamSpotOnlineObjects& bs2){
